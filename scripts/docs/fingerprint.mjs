...
export async function fingerprint(root=process.cwd()){const list=(await Promise.all(roots.map(x=>files(resolve(root,x))))).flat().sort();const h=createHash("sha256");for(const f of list){const r=relative(root,f).split(sep).join("/");h.update(r);h.update((await readFile(f,"utf8")).replace(/\r\n/g,"\n"));}return h.digest("hex")}

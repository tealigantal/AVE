...
export async function acceptanceEntry(phase: string, output: string, manifest = process.env.AVE_REAL_MEDIA_MANIFEST) {
  ...
  if (material?.blueprint_id === 'duration-60s-v1') {
    const revisedSeconds = 60 - material.feedback.trim_pts / material.evidence.find(e => e.id === material!.feedback.evidence)!.timescale;
    await writeFile(resolve(root, '素材准备.md'), `# 真实单素材检查\n\n本轮按用户指定使用现有真实原片，目标为两份 60 秒方案与一份 ${revisedSeconds} 秒修改后成片；实际编码状态见开始验收页。保留素材授权和原声，片段范围不重复填充。本轮不代表六段独立原片的两分钟主案例验收，人工观看结果仍待填写。\n`);
  }

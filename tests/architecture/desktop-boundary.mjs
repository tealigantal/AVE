...
if (duplicateStoryGuard < 0 || duplicateStoryGuard > confirmation.indexOf("showMessageBox(options)")) throw new Error("duplicate Story approval must close before native confirmation");
...
if (!confirmation.includes('action === "feedback.reject"') || !dialogs.includes("AVE_ELECTRON_PRODUCT_REVIEW_REJECT_CONFIRM") || dialogs.includes('action === "intent.approve"\n    && process.env.AVE_ELECTRON_PRODUCT_REVIEW')) throw new Error("Product review automation may confirm only exact feedback rejection");

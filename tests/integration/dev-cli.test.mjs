...
try {
  ...
  const migrated = JSON.parse((await run(process.execPath, ["--import", "tsx", cli, "migrate-project", root])).stdout); assert.equal(migrated.ok, true); assert.equal(migrated.integrity, "ok"); assert.equal(migrated.schema_version, 19);

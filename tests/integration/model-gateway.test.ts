...
let calls = 0;
const request = { request_id: "model-1", project_id: "project-1", related_artifact_id: "artifact-1", provider: "qwen", model: "qwen-test", model_snapshot: "snap-1", prompt_version: "story:v1", input: { context: { text: "hello" }, media: [] }, privacy_class: "internal" as const, structured_output: true };
...
const unknownUsage = await runModel({ ...request, request_id: "model-unknown-usage" }, async () => ({ ok: true })); assert.equal(unknownUsage.token_usage, undefined);
...
const fakeFetch = async (url: RequestInfo | URL, init?: RequestInit) => { assert.match(String(url), /chat\/completions$/); assert.equal((init?.headers as Record<string, string>).authorization, "Bearer test-key"); return new Response(JSON.stringify({ model: "qwen-test-snapshot", choices: [{ message: { content: '{"answer":1}' }, finish_reason: "stop" }], usage: { prompt_tokens: 2, completion_tokens: 3, total_tokens: 5 } }), { status: 200, headers: { "content-type": "application/json" } }); };
...
assert.deepEqual((await qwen.complete(request)).token_usage, { input: 2, output: 3, total: 5 }); assert.equal((await deepseek.complete({ ...request, provider: "deepseek" })).model_snapshot, "qwen-test-snapshot");

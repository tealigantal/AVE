...
try {
  ...
  assert.equal(plans.every((item) => item.adapter_version === "v5" && item.capability_snapshot.adapter_version === "v5"), true, "enabled Ducking must publish only corrected adapter identities");
  ...
  assert.equal(outputs.every((item) => item.worker_version === "ave-worker-host-r15"), true);

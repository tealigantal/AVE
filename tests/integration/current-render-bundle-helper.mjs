...
export function registerCurrentRenderFixture(session, projectId, { renderId, outputPath, timelineVersion = 1, binding } = {}) {
  ...
  const result = (target) => ({ render_result_id: `${renderId}-${target}`, render_id: renderId, target, timeline_version: timelineVersion, graph_hash: semanticHash, render_graph: { target }, original_refs: [], proxy_refs: [], profile: { name: "current-test", ...(binding ? { stage2_execution_binding: binding } : {}) }, worker_version: "ave-worker-host-r15", ffmpeg_version: "ffmpeg-test", output_path: outputPath, output_hash: outputHash });
  const output = (target) => ({ schema_version: 2, render_id: renderId, target, semantic_graph_hash: semanticHash, execution_plan_id: plans[target].plan_id, cache_key: plans[target].cache_key, output_hash: outputHash, worker_version: "ave-worker-host-r15", backend_version: "ffmpeg-test", diagnostics: [] });

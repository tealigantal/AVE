...
const harnessDialog = new Proxy(dialog, {
  get(target, property, receiver) {
    if (property === "showMessageBox") return async (...args: unknown[]) => {
      const options = args.at(-1) as { buttons?: readonly string[] } | undefined;
      if (harnessMode === "product" && options?.buttons?.[1] === "确认拒绝") return { response: 1, checkboxChecked: false };
      throw new Error("Electron harness refused a non-feedback-rejection native confirmation");
    };

...
try {
  ...
  const corruptedImmutableBytes = Buffer.alloc(immutableBytes.byteLength, 0x79), corruptedImmutableDigest = createHash("sha256").update(corruptedImmutableBytes).digest("hex");
  await chmod(immutableMediaPath, 0o666); await writeFile(immutableMediaPath, corruptedImmutableBytes); await utimes(immutableMediaPath, immutableFileTime, immutableFileTime); await chmod(immutableMediaPath, 0o400);
  const disguisedImmutableStat = await stat(immutableMediaPath); assert.equal(disguisedImmutableStat.size, immutableMetadata.file_stat.size); assert.equal(disguisedImmutableStat.mtimeMs, immutableMetadata.file_stat.mtime_ms, "fixture must preserve immutable stat metadata so content hashing proves corruption"); assert.equal(process.platform === "win32" ? disguisedImmutableStat.mode & 0o222 : disguisedImmutableStat.mode & 0o777, process.platform === "win32" ? 0 : 0o400, "fixture must restore immutable protection so mode checks cannot mask content corruption");
  let immutableFingerprintChecks = 0;
  workerPort.submit = async (taskType: string, input: any, control: unknown) => { const result = await originalWorkerSubmit(taskType, input, control); if (taskType === "media.fingerprint.v1" && input?.input_path === immutableMediaPath) { const output = (result as any).outputs?.find((candidate: any) => candidate.kind === "media.fingerprint"); assert.equal(output?.digest, corruptedImmutableDigest, "Worker must return the full SHA-256 of the corrupted immutable bytes"); immutableFingerprintChecks += 1; } return result; };
  try {
    ...
    assert.ok(immutableFingerprintChecks > 0, "full content hashing must detect corruption after size, mtime and immutable mode are restored");

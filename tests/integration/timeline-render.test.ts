...
try {
  ...
  assert.equal((retriedRender.preview as any).metrics.worker_version, "ave-worker-host-r15");
  assert.equal((retriedRender.master as any).metrics.worker_version, "ave-worker-host-r15");
  ...
  assert.equal(result.worker_version, "ave-worker-host-r15");
  ...
  assert.equal(plans.every((plan) => plan.adapter_version === "v5"), true, "every persisted plan must use the current adapter identity");

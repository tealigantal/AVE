...
try {
  ...
  assert.equal((retriedRender.preview as any).metrics.worker_version, "ave-worker-host-r14");
  assert.equal((retriedRender.master as any).metrics.worker_version, "ave-worker-host-r14");
  ...
  assert.match(result.worker_version, /^ave-worker-host-r14/);
  ...
  assert.equal(plans.every((plan) => plan.adapter_version === "v4"), true, "every persisted plan must use the current adapter identity");
  ...
  transitionHost.initializeTimeline([{ track_id: "v1", kind: "video", clips: [{ clip_id: "left", source: sourceRange(assetA, 0n, 20n, 30n), timeline_start: 0n, timeline_duration: 20n }, { clip_id: "right", source: sourceRange(assetB, 0n, 20n, 30n), timeline_start: 15n, timeline_duration: 20n }], transitions: [{ transition_id: "blocked-transition", kind: "spin", from_clip_id: "left", to_clip_id: "right", timeline_start: 15n, timeline_duration: 5n }] }]);
  await assert.rejects(transitionHost.renderTimeline({ sources: [{ asset_ref: assetA, original_ref: red, proxy_ref: redProxy, source_timescale: 30n }, { asset_ref: assetB, original_ref: blue, proxy_ref: blueProxy, source_timescale: 30n }], profile: { name: "transition-blocker", width: 64, height: 64 } }), /RENDER_RESOLVER_BLOCKED:TRANSITION_KIND_RENDER_UNSUPPORTED/);
  const transitionPlans = (transitionHost.listRenderManifests() as any[]).filter((manifest) => manifest.manifest_type === "execution_plan" && manifest.value.diagnostics?.some((diagnostic: any) => diagnostic.code === "TRANSITION_KIND_RENDER_UNSUPPORTED"));

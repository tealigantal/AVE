...
import { PresetRegistry, builtInPresetDefinitions, createBuiltInPresetRegistry, migratePresetSelection, presetDigest, resolveCreativeSkill, type CreativeSkillOutput, type PresetDefinition, type PresetResolutionContext } from "../../packages/core/preset-core/src/public.js";
...
const malicious = resolveCreativeSkill({ ...output(), commands: [{ type: "raw_backend", filter: "movie=/etc/passwd" }] } as any, registry, context());
assert.equal(malicious.status, "blocked");
assert.equal(malicious.diagnostics.some((diagnostic) => diagnostic.code === "CREATIVE_SKILL_OUTPUT_UNKNOWN_FIELD"), true);
assert.equal(malicious.commands.length, 0);
...
assert.throws(() => new PresetRegistry([{ ...localVersion, parameter_schema: [{ ...localVersion.parameter_schema[0], executable_code: "return process.env" } as any] }]), /PRESET_PARAMETER_SCHEMA_UNKNOWN_FIELD/);
...
assert.equal(resolveCreativeSkill(null as any, registry, context()).diagnostics[0].code, "CREATIVE_SKILL_OUTPUT_INVALID");
assert.equal(resolveCreativeSkill(output({ selections: [{ ...output().selections[0], parameters: { x: null as any } }] }), registry, context()).diagnostics.some((diagnostic) => diagnostic.code === "PRESET_PARAMETER_INVALID"), true, "an explicit null must not be replaced by a default");

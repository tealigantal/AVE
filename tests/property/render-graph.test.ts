...
assert.ok(blockerFor({ grade: { grade_id: "wide-color", brightness: 0.1, context: { input_space: "rec2020", working_space: "rec709", output_space: "rec709", bit_depth: 10, range: "full" } } }).includes("COLOR_CONTEXT_RENDER_UNSUPPORTED"), "non-default color context must block before Worker submission");
...
assert.ok(blockerFor({ time_map: { map_id: "change-pitch", pitch_policy: "change", segments: [{ segment_id: "speed", timeline_start: 0n, timeline_end: 30n, source_start: 0n, source_end: 30n, mode: "speed", speed_numerator: 1n, speed_denominator: 1n }] } }).includes("TIME_MAP_PITCH_POLICY_UNSUPPORTED"), "unsupported pitch policy must block before Worker submission");
const transitionTimeline = { version: 1, tracks: [{ track_id: "transition-track", kind: "video" as const, clips: [{ ...clip("transition-left", assetA), timeline_start: 0n, timeline_duration: 30n }, { ...clip("transition-right", assetB), timeline_start: 30n, timeline_duration: 30n }], transitions: [{ transition_id: "transition", kind: "dissolve", from_clip_id: "transition-left", to_clip_id: "transition-right", timeline_start: 25n, timeline_duration: 5n }] }] };
const transitionPlan = resolveExecutionPlan(buildTimelineRenderGraph(transitionTimeline, sources, "master"), "master");
assert.ok(transitionPlan.diagnostics.some((diagnostic) => diagnostic.code === "TRANSITION_HANDLE_EXECUTION_UNSUPPORTED"), "transitions must block until source handles can be executed without losing incoming media");
const vlogTimeline = { version: 1, master_loudness: { schema_version: 1 as const, enabled: true, target_lufs: -14, true_peak_db: -1, tolerance_lufs: 1 }, dialogue_music_ducking: { schema_version: 1 as const, enabled: true, threshold_db: -30, ratio: 8, attack_ms: 20, release_ms: 350, max_reduction_db: 12 }, tracks: [{ track_id: "vlog-video", kind: "video" as const, clips: [{ ...clip("vlog-clip", assetA), static_reframe: { schema_version: 1 as const, mode: "crop_fill" as const, focal_x: 0.6, focal_y: 0.4 }, boundary_fades: { schema_version: 1 as const, video_fade_in: { value: 5n, timescale: 30n }, audio_fade_out: { value: 5n, timescale: 30n } } }] }, { track_id: "dialogue", kind: "audio" as const, clips: [clip("dialogue-clip", assetB)], audio_routing: [{ routing_id: "dialogue-routing", source_clip_id: "dialogue-clip", bus: "dialogue" as const }] }, { track_id: "music", kind: "audio" as const, clips: [clip("music-clip", assetC)], audio_routing: [{ routing_id: "music-routing", source_clip_id: "music-clip", bus: "music" as const }] }] };
const vlogPreview = buildTimelineRenderGraph(vlogTimeline, sources, "preview", { name: "vertical", width: 360, height: 640 });
const vlogMaster = buildTimelineRenderGraph(vlogTimeline, sources, "master", { name: "vertical", width: 360, height: 640 });
...
const badProfilePlan = resolveExecutionPlan(buildTimelineRenderGraph(vlogTimeline, sources, "master", { name: "landscape", width: 640, height: 360 }), "master");
assert.ok(badProfilePlan.diagnostics.some((diagnostic) => diagnostic.code === "STATIC_REFRAME_9_16_PROFILE_REQUIRED"));
assert.ok(transitionPlan.diagnostics.some((diagnostic) => diagnostic.code === "TRANSITION_HANDLE_EXECUTION_UNSUPPORTED"), "single-clip fades must not unlock real transitions");

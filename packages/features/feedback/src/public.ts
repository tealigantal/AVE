...
type RationalTimeInput = Readonly<{ schema_version: 1; value: number; timescale: number }>;
type SourceRangeInput = Readonly<{ asset_id: string; start: RationalTimeInput; end: RationalTimeInput }>;
export type FeedbackRevisionDiagnosisInput = Readonly<{
  ...
  target: Readonly<{ track_id: string; clip_id: string; original_source: SourceRangeInput; proposed_source: SourceRangeInput; trim_duration: RationalTimeInput }>;
...
const validTime = (value: RationalTimeInput): boolean => value?.schema_version === 1 && Number.isSafeInteger(value.value) && value.value >= 0 && Number.isSafeInteger(value.timescale) && value.timescale > 0;
...
export function validateFeedbackDiagnosisV2(value: FeedbackDiagnosisV2): void {
  ...
  const original = value.target.original_source, proposed = value.target.proposed_source, trim = value.target.trim_duration;
  const exactTrim = validTime(trim) && trim.value > 0 && BigInt(original.end.value) * BigInt(proposed.end.timescale) * BigInt(trim.timescale) - BigInt(proposed.end.value) * BigInt(original.end.timescale) * BigInt(trim.timescale) === BigInt(trim.value) * BigInt(original.end.timescale) * BigInt(proposed.end.timescale);
  if (original.asset_id !== proposed.asset_id || !/^asset:sha256:[0-9a-f]{64}$/.test(original.asset_id) || !validTime(original.start) || !validTime(original.end) || !validTime(proposed.start) || !validTime(proposed.end) || !exactTrim || compareTime(original.start, original.end) >= 0 || compareTime(proposed.start, proposed.end) >= 0 || compareTime(proposed.start, original.start) !== 0 || compareTime(proposed.end, original.end) >= 0) throw new Error("FEEDBACK_TRIM_NOT_STRICT_INWARD");

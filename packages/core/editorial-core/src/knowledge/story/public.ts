...
function assertStoryBeatCount(beats: readonly StoryBeatCandidate[] | StoryProposalV2["beats"], duration: DurationFeasibilityV1): void {
  if (!Number.isSafeInteger(duration.planned_beat_count) || duration.planned_beat_count < 1) throw new Error("story planned Beat count is invalid");
  if (beats.length !== duration.planned_beat_count) throw new Error(`story Beat count must exactly match Duration Feasibility:${beats.length}:${duration.planned_beat_count}`);
}
export function evaluateStoryProposal(input: StoryProposalInput, direction: DirectionCardV1, contract: CreativeContractV2, pack: MaterialEvidencePackV1, coverage: CoverageMatrix, evaluations: readonly SkillEvaluationV1[], duration: DurationFeasibilityV1): StoryProposalV2 {
  ...
  validateCurrentContext(contract, pack, duration, input.contract_ref, input.material_pack_ref, input.duration_feasibility_ref);
  assertStoryBeatCount(input.beats, duration);
...
export function approveStoryProposalV2(proposals: readonly StoryProposalV2[], input: StoryApprovalInput, contract: CreativeContractV2, duration: DurationFeasibilityV1): Readonly<{ decision: DecisionRecordV1; plan: ApprovedStoryPlanV2 }> {
  if (proposals.length < 2) throw new Error("story approval requires comparable proposals");
  if (new Set(proposals.map((proposal) => proposal.proposal_id)).size !== proposals.length) throw new Error("story approval requires unique proposals");
  const selected = proposals.find((proposal) => proposal.proposal_id === input.selected_proposal_id);
  if (!selected || proposals.some((proposal) => proposal.status !== "candidate") || !input.decision_id.trim() || !input.plan_id.trim() || !input.actor_id.trim() || !input.reason.trim() || !/^[0-9a-f]{64}$/.test(input.review_digest) || !isStrictComparableDateTime(input.approved_at)) throw new Error("story approval input is invalid");
  if (duration.result !== "feasible" || duration.feasibility_id !== selected.duration_feasibility_ref.object_id || duration.object_version !== selected.duration_feasibility_ref.object_version || editorialObjectDigest(duration) !== selected.duration_feasibility_ref.digest) throw new Error("story approval Duration Feasibility is stale or rebound");
  ...
  proposals.forEach((proposal) => {
    assertStoryBeatCount(proposal.beats, duration);
    if (new Set(proposal.beats.map((beat) => beat.beat_id)).size !== proposal.beats.length) throw new Error("story candidate Beat ids must be unique");
    if (!equalFraction(sumTime(proposal.beats.map((beat) => beat.target_duration)), fraction(proposal.duration_budget, "story duration budget")) || !equalFraction(fraction(proposal.duration_budget, "story duration budget"), fraction(duration.target_duration, "story feasible target"))) throw new Error("story candidate fails approval hard gates");
  });
  const ranked = rankStoryProposals(proposals);
  ...
  if (selected.evaluation.dimensions.evidence_coverage !== 1 || selected.evaluation.dimensions.duration_fit !== 1 || selected.beats.some((beat) => beat.unresolved_assumptions.length > 0)) throw new Error("story candidate fails approval hard gates");

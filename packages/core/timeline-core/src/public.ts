...
export function validateTimelineDetailed(timeline: Timeline): readonly TimelineValidationIssue[] {
  ...
  for (const track of allTracks) {
    ...
    for (const transition of track.transitions ?? []) {
      ...
      if (!from || !to || fromIndex + 1 !== toIndex || transition.timeline_duration <= 0n || transition.timeline_duration >= from.timeline_duration || transition.timeline_duration >= to.timeline_duration || !explicitOverlap) issues.push({ code: "TRANSITION", id: transition.transition_id, message: `transition requires exact overlap and source handles: ${transition.transition_id}` });
    }

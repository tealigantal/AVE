...
export { evaluateAutomationCurve, validateAutomationCurve, type AutomationCurve, type AutomationKeyframe, type AutomationValue, type BoundaryPolicy, type Interpolation, type Tangent } from "./automation.js";
...
export type TimelineValidationCode = "MEDIA_REF" | "SOURCE_RANGE" | "OVERLAP" | "TRANSITION" | "LOCK" | "CAPTION" | "AUDIO_ROUTING" | "DUPLICATE_ID" | "VERSION" | "TRACK_COMPATIBILITY" | "SEQUENCE" | "CYCLE" | "COMPOUND";
...
function applyCommandUnchecked(timeline: Timeline, command: TimelineCommand): Timeline {
  ...
  if (command.type === "set_speed") return replaceTrack(timeline, updateClip(track, index, { ...track.clips[index], speed: command.speed }));
...
export function validateTimelineDetailed(timeline: Timeline): readonly TimelineValidationIssue[] { const issues: TimelineValidationIssue[] = []; if (!Number.isInteger(timeline.version) || timeline.version < 0) issues.push({ code: "VERSION", message: "timeline version must be a non-negative integer" }); const ids = new Set<string>(); const addId = (id: string, kind: string) => { if (!id || ids.has(id)) issues.push({ code: "DUPLICATE_ID", id, message: `duplicate or empty ${kind} id: ${id}` }); ids.add(id); }; const validAsset = (asset: string) => /^asset:sha256:[0-9a-f]{64}$/.test(asset); const sequences = timeline.sequences ?? []; const sequenceById = new Map(sequences.map((sequence) => [sequence.sequence_id, sequence])); if (timeline.sequence) sequenceById.set(timeline.sequence.sequence_id, timeline.sequence); for (const sequence of sequenceById.values()) { addId(sequence.sequence_id, "sequence"); if (sequence.parent_sequence_id && !sequenceById.has(sequence.parent_sequence_id)) issues.push({ code: "SEQUENCE", id: sequence.sequence_id, message: `sequence parent not found: ${sequence.parent_sequence_id}` }); }
  const parentEdges = new Map<string, string[]>(); for (const sequence of sequenceById.values()) if (sequence.parent_sequence_id) parentEdges.set(sequence.sequence_id, [sequence.parent_sequence_id]); const sequenceVisit = new Set<string>(); const sequenceActive = new Set<string>(); const visitSequence = (id: string): void => { if (sequenceActive.has(id)) { issues.push({ code: "CYCLE", id, message: `sequence cycle: ${id}` }); return; } if (sequenceVisit.has(id)) return; sequenceActive.add(id); for (const next of parentEdges.get(id) ?? []) visitSequence(next); sequenceActive.delete(id); sequenceVisit.add(id); }; for (const id of sequenceById.keys()) visitSequence(id);
  const allTracks = [...timeline.tracks, ...sequences.flatMap((sequence) => sequence.tracks)]; for (const track of allTracks) { addId(track.track_id, "track"); if (track.z_index !== undefined && !Number.isInteger(track.z_index)) issues.push({ code: "TRACK_COMPATIBILITY", id: track.track_id, message: "track z_index must be an integer" }); if (track.opacity !== undefined && (!Number.isFinite(track.opacity) || track.opacity < 0 || track.opacity > 1)) issues.push({ code: "TRACK_COMPATIBILITY", id: track.track_id, message: "track opacity must be in [0, 1]" }); const clipEnds: Array<{ clip: Clip; end: bigint }> = []; for (const clip of track.clips) { addId(clip.clip_id, "clip"); if (!validAsset(clip.source.asset_id)) issues.push({ code: "MEDIA_REF", id: clip.clip_id, message: `invalid media ref: ${clip.source.asset_id}` }); if (clip.source.timescale <= 0n || clip.source.start_pts < 0n || clip.source.end_pts <= clip.source.start_pts) issues.push({ code: "SOURCE_RANGE", id: clip.clip_id, message: `invalid source range: ${clip.clip_id}` }); if (clip.timeline_start < 0n || clip.timeline_duration <= 0n) issues.push({ code: "SOURCE_RANGE", id: clip.clip_id, message: `invalid timeline range: ${clip.clip_id}` }); if (clip.media_kind && clip.media_kind !== track.kind) issues.push({ code: "TRACK_COMPATIBILITY", id: clip.clip_id, message: `clip media kind does not match ${track.kind} track` }); if (clip.kind === "nested" && (!clip.nested_sequence_id || !sequenceById.has(clip.nested_sequence_id))) issues.push({ code: "SEQUENCE", id: clip.clip_id, message: "nested clip requires an existing sequence" }); if (clip.kind === "compound" && (!clip.compound_clip_ids?.length || clip.compound_clip_ids.includes(clip.clip_id))) issues.push({ code: "COMPOUND", id: clip.clip_id, message: "compound clip needs non-self children" }); const end = clipEnd(clip); for (const prior of clipEnds) if (clip.timeline_start < prior.end && prior.clip.timeline_start < end) issues.push({ code: "OVERLAP", id: clip.clip_id, message: `clips overlap: ${prior.clip.clip_id}, ${clip.clip_id}` }); clipEnds.push({ clip, end }); } for (const gap of track.gaps ?? []) { addId(gap.gap_id, "gap"); if (gap.timeline_start < 0n || gap.timeline_duration <= 0n) issues.push({ code: "SOURCE_RANGE", id: gap.gap_id, message: `invalid gap range: ${gap.gap_id}` }); } for (const caption of track.captions ?? []) { addId(caption.caption_id, "caption"); if (!caption.text.trim() || caption.timeline_start < 0n || caption.timeline_duration <= 0n) issues.push({ code: "CAPTION", id: caption.caption_id, message: `invalid caption: ${caption.caption_id}` }); if (track.kind !== "video") issues.push({ code: "TRACK_COMPATIBILITY", id: caption.caption_id, message: "caption must be on video track" }); } const clipIds = new Set(track.clips.map((clip) => clip.clip_id)); for (const transition of track.transitions ?? []) { addId(transition.transition_id, "transition"); if (!clipIds.has(transition.from_clip_id) || !clipIds.has(transition.to_clip_id) || transition.from_clip_id === transition.to_clip_id || transition.timeline_duration <= 0n) issues.push({ code: "TRANSITION", id: transition.transition_id, message: `invalid transition: ${transition.transition_id}` }); } for (const effect of track.effects ?? []) { addId(effect.effect_id, "effect"); if (!clipIds.has(effect.clip_id)) issues.push({ code: "TRACK_COMPATIBILITY", id: effect.effect_id, message: `effect clip not found: ${effect.clip_id}` }); } for (const keyframe of track.keyframes ?? []) { addId(keyframe.keyframe_id, "keyframe"); if (keyframe.time < 0n || !clipIds.has(keyframe.target_id)) issues.push({ code: "SOURCE_RANGE", id: keyframe.keyframe_id, message: `invalid keyframe: ${keyframe.keyframe_id}` }); } for (const routing of track.audio_routing ?? []) { addId(routing.routing_id, "routing"); if (track.kind !== "audio" || !clipIds.has(routing.source_clip_id)) issues.push({ code: "AUDIO_ROUTING", id: routing.routing_id, message: `invalid audio routing: ${routing.routing_id}` }); } for (const lock of track.locks ?? []) { addId(lock.lock_id, "lock"); if (lock.start < 0n || lock.end <= lock.start || !lock.owner) issues.push({ code: "LOCK", id: lock.lock_id, message: `invalid lock: ${lock.lock_id}` }); } } return issues; }

...
Remove the repository-wide high-severity `fast-uri` advisory without weakening the audit gate or changing AVE product behavior. The dependency-only repair must preserve the accepted Preset work, all existing blocked capability boundaries, and the complete local and GitHub validation paths.
...
Allowed change scope for this maintenance task is `pnpm-lock.yaml`, `pnpm-workspace.yaml` and `docs/**`; `package.json` is used only if pnpm configuration cannot reliably hold the patched version. Application source, contracts, workflows, and security thresholds are unchanged.
...
The high-severity advisory is resolved locally without changing application source, protocols, workflows or audit thresholds. The workspace override makes the compatible patched transitive resolution explicit, while the regenerated lockfile makes frozen CI installation deterministic. New immutable regression Evidence carries the changed fingerprint and preserves all prior accepted, tested and blocked capability boundaries. Remote GitHub confirmation remains the publication step.

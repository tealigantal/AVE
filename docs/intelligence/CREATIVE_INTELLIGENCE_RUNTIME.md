...
| Render and QC | committed Timeline | Preview/Master RenderGraph, outputs and QC | existing Project Host/Worker path | same semantic graph, source identity, ExecutionPlan, output/QC checks | explicit blocker bundle; never label missing output successful |

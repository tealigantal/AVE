...
Adopt option 3 for successful applications. Project Host resolves and simulates the full ordered selection set, checks the supplied base Timeline version and creates one CommitPlan. It then rebuilds the actual Preview and Master RenderGraphs from the candidate Timeline and resolves both ExecutionPlans before commit. Project Storage registers the immutable `preset_application` object reference in the same SQLite transaction as the Timeline version and CommitPlan. The record contains base/final version, exact definition pins and digests, resolved parameters, application context, canonical command payload and hashes, asset/license/trust decisions, declared capabilities, target-neutral semantic expectation, actual Preview/Master decisions and explicit links from every declared semantic to executing RenderGraph node IDs.
...
Preview and Master must have the same target-neutral semantic expectation. Target-specific source/profile/adapter decisions may differ only when they remain semantically equivalent. Bake succeeds only for a trusted content-addressed artifact with acceptable licensing; otherwise it blocks. No node may be silently omitted.
...
`commitTimelinePlan` gains an optional atomic artifact-registration input while preserving old callers. Failed pre-transaction object staging may leave an unreferenced content-addressed object, which is non-authoritative and recoverable through the existing orphan mechanism. Blocked application records are intentionally retained as audit evidence.

...
for (const path of await walk(schemaRoot)) { const schema = JSON.parse(await readFile(path, "utf8")); if (seen.has(schema.$id)) throw new Error(`duplicate schema id: ${schema.$id}`); seen.add(schema.$id); if (!schema.$id.includes(".v1.")) throw new Error(`schema lacks explicit major version: ${path}`); }
console.log(`compatibility check passed (${seen.size} unique v1 ids)`);

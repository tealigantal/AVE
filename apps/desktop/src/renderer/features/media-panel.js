export function mediaPanel(actions, state) {
  ...
  for (const item of media) { const row = document.createElement("button"); row.type = "button"; row.className = `media-row ${state.selectedAssetId === item.asset_id ? "selected" : ""}`; const identity = document.createElement("strong"); identity.textContent = item.asset_id; const location = document.createElement("span"); location.textContent = `${item.location_type ?? "media"} · ${item.permission_state ?? "已登记"}`; row.append(identity, location); row.addEventListener("click", () => actions.selectAsset(item.asset_id)); list.append(row); }

declare module "electron" {
  export const app: { whenReady(): Promise<void>; getPath(name: string): string; on(event: string, listener: (event: { preventDefault(): void }) => void): void; quit(): void };
  ...
  export class BrowserWindow { readonly id: number; readonly webContents: { send(channel: string, payload: unknown): void }; constructor(options: unknown); loadFile(path: string): Promise<void>; loadURL(url: string): Promise<void>; on(event: string, listener: (event: { preventDefault(): void }) => void): void; static getAllWindows(): BrowserWindow[]; static fromWebContents(webContents: unknown): BrowserWindow | null; }
